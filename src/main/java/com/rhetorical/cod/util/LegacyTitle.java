...
public class LegacyTitle {
	...
	public static void sendTitle(Player player, String text, int fadeInTime, int showTime, int fadeOutTime, ChatColor color) {
		try {
			...
			Bukkit.getLogger().severe("Could not get title in COM-Warfare!");
			ex.printStackTrace();
		}
	}
	...
	private static void sendPacket(Player player, Object packet) {
		try {
			...
			Bukkit.getLogger().severe("Could not send title packet in COM-Warfare!");
			ex.printStackTrace();
		}
	}
	...
	private  static Class<?> getNMSClass(String name) {
		try {
			...
			Bukkit.getLogger().severe(String.format("Could not get nms class %s in COM-Warfare!", name));
			ex.printStackTrace();
		}
		return null;
	}
