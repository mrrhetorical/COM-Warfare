...
class DependencyManager {
	...
	void downloadDependencies() throws IOException {
		if (Bukkit.getServer().getPluginManager().getPlugin("McTranslatePlusPlus") == null) {
			Main.getConsole().sendMessage(Main.getPrefix() + "Starting download of \"McTranslate++\" jar.");
			...
			Main.getConsole().sendMessage(Main.getPrefix() + "Successfully downloaded \"McTranslate++\" jar.");
			Main.getConsole().sendMessage(Main.getPrefix() + "Restarting Server...");
